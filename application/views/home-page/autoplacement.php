                  ...
                  <div class="card-body StudentViewTextInfo">
                    ...
                    <div class="tab-content tab-bordered" id="myTab3Content">
                      ...
                      <div class="tab-pane fade show" id="automaticPlacement" role="tabpanel" aria-labelledby="home-tab2">
                        ...
                        <div class="listAuto card-body" id="helloAuto"> </div>
                      </div>
                    </div>
                  </div>
                </div>
...
<script type="text/javascript">
  $(document).on('click', '.placesiec', function() {
    ...
    $.ajax({
      url: "<?php echo base_url(); ?>Automaticplacement/insertsection/",
      method: "POST",
      data: ({
        stu_id: stu_id,
...
<script type="text/javascript">
  $('#comment_formManual').on('submit', function(event) {
    ...
    if ($('#grade2place').val() != '') {
      ...
      $.ajax({
        url: "<?php echo base_url(); ?>Automaticplacement/filter_grade4placement/",
        method: "POST",
        data: form_data,
        beforeSend: function() {
...
<script type="text/javascript">
  $(document).ready(function() {  
    $("#branchManual").bind("change", function() {
      $.ajax({
        type: "POST",
        url: "<?php echo base_url(); ?>Filter_thisgrade_from_branch/",
  ...
  $(document).ready(function() {  
    $("#grands_branchit").bind("change", function() {
      $.ajax({
        ...
        beforeSend: function() {
          $('#grade2place').html( '<img src="<?php echo base_url(); ?>/img/loader.gif" alt="" width="34" height="34" id="loa">' );
        },
        success: function(data) {
          $("#grade2place").html(data);
        }
      });
    });
  });
...
<script type="text/javascript">
  $('#grade2place').on('change', function(event) {
    ...
    $.ajax({
      url: "<?php echo base_url(); ?>Automaticplacement/checkPlacementFound/",
      method: "POST",
      data: ({
    ...
    .then((willDelete) => {
      if (willDelete) {
        if ($('#grade2place').val() != '') {
          var form_data = $(this).serialize();
          $.ajax({
            url: "<?php echo base_url(); ?>Automaticplacement/filterGrade4AutoPlacement/",
            method: "POST",
            data: form_data,
            beforeSend: function() {
