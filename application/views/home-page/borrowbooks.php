...
<script type="text/javascript">
  $(document).ready(function(){
    $('#empTableGS').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
        'url':'<?=base_url()?>borrowbooks/fetch_book_borrow/'
      },
      'columns': [
         { data: 'book_name' },
         { data: 'book_grade' },
  ...
  $(document).on('click', '#borrowThisBookName', function() {
    ...
    $.ajax({
      method: "POST",
      url: "<?php echo base_url(); ?>borrowbooks/borrow_book_name/",
      data: ({
        stockid: stockid
      }),
      cache: false,
  ...
  $(document).on('submit', '#submiteBorrowBookName', function(e) {
    ...
    if ($('#dateReturnBook').val() != '') {
      $.ajax({
        method: "POST",
        url: "<?php echo base_url(); ?>borrowbooks/borrowThis_bookName/",
  ...
  $(document).on('click', '.removeBorrowRequest', function() {
    ...
    .then((willDelete) => {
      if (willDelete) {
        ...
        $.ajax({
          method: "POST",
          url: "<?php echo base_url(); ?>borrowbooks/removerequest_borrow_book_name/",
          data: ({
            stockid: stockid
          }),
          cache: false,
