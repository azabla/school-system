                ...
                <div class="card">
                  <div class="card-body StudentViewTextInfo">
                    <ul class="nav nav-tabs" id="myTab2" role="tablist">
                      <li class="nav-item">
                        <a class="nav-link active" id="home-tab1" data-toggle="tab" href="#feedSubject" role="tab" aria-selected="true"> Add Unit Admin Placement</a>
                      </li>
                      <li class="nav-item">
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    {
      $.ajax({
        url:"<?php echo base_url(); ?>unitleaderplacement/fetch_placement/",
        method:"POST",
        beforeSend: function() {
    ...
    $('#save_placement').on('submit', function(event) {
      ...
      $("input[name='grade']:checked").each(function(i){
        ...
        $.ajax({
        method: "POST",
        url: "<?php echo base_url(); ?>unitleaderplacement/post_placement/",
        data: ({
          id: id,
  ...
  $(document).on('click', '#delete_directorunitplacement', function()
  {
    var staff_placement=$(this).attr("value");
    swal({
      title: 'Are you susre you want to delete this unit admin Placement?',
      text: '',
      icon: 'warning',
      buttons: true,
      dangerMode: true,
    ...
    .then((willDelete) => {
      if (willDelete) {
        $.ajax({
          method: "POST",
          url: "<?php echo base_url(); ?>unitleaderplacement/deletePlacement/",
          data: ({
            staff_placement: staff_placement
          }),
          cache: false,
          beforeSend: function() {
            $('.delete_directorunitplacement').html( '<img src="<?php echo base_url(); ?>/img/loader.gif" alt="" width="14" height="14" id="loa">'
            );
          },
          success: function(html){
           $('.delete_directorunitplacement').fadeOut('slow');
           load_data();
          }
        });
      }
