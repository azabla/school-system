...
<body>
                ...
                <form id="mydocument" method="POST">
                  <div class="row">
                    ...
                    <div class="col-lg-5 12">
                      <button type="submit"  name="post" class="btn btn-primary btn-block">Save File</button>
                    </div>
                  </div>
                </form>
  ...
  <script type="text/javascript">
    $(document).ready(function(){
      ...
      $.ajax({
        method: "POST",
        url: "<?php echo base_url(); ?>document/fetch_this_report_toedt/",
        data: ({
          rid: rid
        }),
        cache: false,
      ...
      $(document).on('click', '#addnewsend_new_my_task_report', function() {
        $.ajax({
          method: "POST",
          url: "<?php echo base_url(); ?>document/new_submit_report_page/",
          cache: false,
          beforeSend: function() {
      ...
      {
        $.ajax({
          url:"<?php echo base_url(); ?>document/fetch_my_tasks_report/",
          method:"POST",
          beforeSend: function() {
      ...
      $(document).on('submit', '#submit_this_newReport', function(e) {
          ...
          if (reportvalue.length  == 0  && $('#importReportFile_gs').val()=='') {
            ...
            $.ajax({
              method: "POST",
              url: "<?php echo base_url(); ?>document/submit_this_newReport/",
        ...
        $(document).on('submit', '#submit_this_updateReport', function(e) {
          ...
          if(reportvalue.length  == 0  && $('#my_Report_Name_update').val()=='') {
            ...
            $.ajax({
              method: "POST",
              url: "<?php echo base_url(); ?>document/submit_updated_this_newReport/",
              data:new FormData(this),
              processData:false,
              contentType:false,
              cache: false,
        ...
        $(document).on('click', '#remove_this_report_file', function(e) {
          ...
          .then((willDelete) => {
            if (willDelete) {
              ...
              $.ajax({
                method: "POST",
                url: "<?php echo base_url(); ?>document/remove_this_report_file/",
                data: ({
                  removedId: removedId
                }),
                cache: false,
      ...
      function load_tasks()
        {
          $.ajax({
            url:"<?php echo base_url(); ?>document/fetch_my_tasks/",
            method:"POST",
            beforeSend: function() {
        ...
        {
          $.ajax({
            url:"<?php echo base_url(); ?>document/fetchdocuments/",
            method:"POST",
            beforeSend: function() {
        ...
        $('#mydocument').on('submit', function(e) {
          ...
          if($('#mydoc').val() =='')
          {
            ...
            $.ajax({
            method: "POST",
            url: "<?php echo base_url(); ?>document/postdocuments/",
            data:new FormData(this),
            processData:false,
            contentType:false,
            cache: false,
  ...
  $(document).ready(function() {
     $(document).on('click', '.deletemydocument', function() {
      ...
      if (confirm("Are you sure you want to delete this file permanently ?")) 
      {
        $.ajax({
          method: "POST",
          url: "<?php echo base_url(); ?>document/Deletedocuments",
          data: ({
            id: id
          }),
          cache: false,
