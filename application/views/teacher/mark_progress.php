                  ...
                  <div class="card-body StudentViewTextInfo">
                    ...
                    <div class="tab-content tab-bordered" id="myTab3Content">
                      <div class="tab-pane fade show active" id="sectionMarkProgress" role="tabpanel" aria-labelledby="home-tab1">
                        ...
                        <div class="col-md-8 col-12">
                          <div class="row">
                            <?php foreach($gre as $row){ ?>
                              <div class="col-md-3 col-4">
                                <input type="checkbox" class="studentGradeSecJoss" name="studentGradeSecJoss" value="<?php echo $row->grade ?>"/><?php echo $row->grade?>
                              </div>
                            <?php } ?>
                          </div>
                        </div>
                        <div class="col-md-4 col-12">
                          <button class="btn btn-primary btn-block viewprogress">
                            View Progress
                          </button>
                        </div>
                      </div>
                      <div class="listprogress" id="helloo"> </div>
                    </div>
                    <div class="tab-pane fade show" id="gradeMarkProgress" role="tabpanel" aria-labelledby="home-tab2">
                      ...
                      <div class="row">
                        <div class="col-md-8 col-12">
                          <div class="row">
                            <?php foreach($grade_dir as $grades){ ?>
                              <div class="col-md-3 col-4">
                                <input type="checkbox" class="studentGradeCrossProgress" name="studentGradeCrossProgress" value="<?php echo $grades->grade ?>"/><?php echo $grades->grade?>
                              </div>
                            <?php } ?>
                          </div>
                        </div>
...
<script>
  $(document).on('click', '.viewGradeProgress', function() {
    ...
    $.ajax({
      method: "POST",
      url: "<?php echo base_url(); ?>Mymarkprogress/Gradecrossprogress/",
      data: ({
        grade: grade
      }),
      cache: false,
...
<script>
  $(document).on('click', '.viewprogress', function() {
    ...
    $.ajax({
      method: "POST",
      url: "<?php echo base_url(); ?>Mymarkprogress/Check_markprogress/",
      data: ({
        grade: grade
      }),
      cache: false,
      beforeSend: function() {
        $('.listprogress').html( 'Loading...<img src="<?php echo base_url(); ?>/img/loader.gif" alt="" width="34" height="34" id="loa">' );
      },
      success: function(html){
        $('.listprogress').html(html);
      }
    }); 
  });
</script>
