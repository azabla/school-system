...
<body>
          ...
          <div class="section-body"> 
            ...
            <div class="card">
              <div class="card-body StudentViewTextInfo">
                  <h5 class="header-title">Annual Leave Request</h5>
                  <div class="row">
                    <div class="col-12">
  ...
  <script type="text/javascript">
    ...
    {
      $.ajax({
        url:"<?php echo base_url(); ?>annualrequest/fetch_myrequested_form/",
        method:"POST",
        beforeSend: function() {
    ...
    $('#submit_leaving_form').on('submit', function(event) {
      ...
      if($("#leaveType").val()!='' && $("#fromDate").val()!=$("#toDate").val() && $("#returnDate").val() > $("#toDate").val() && $("#returnDate").val() > $("#fromDate").val()){
        ...
        if(totalRemaning > total_remainig){
          ...
          $.ajax({
            method: "POST",
            url: "<?php echo base_url(); ?>annualrequest/submit_request/",
            data: ({
              LeaveType: LeaveType,
              fromDate:fromDate,
              toDate:toDate,
    ...
    $(document).on('click', '#cancelRequest', function(event) { 
      ...
      $.ajax({
        method: "POST",
        url: "<?php echo base_url(); ?>annualrequest/delete_request/",
        data: ({
          id: id
        }),
        cache: false,
