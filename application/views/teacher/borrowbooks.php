...
<script type="text/javascript">
  $(document).ready(function(){
    $('#empTableGS').DataTable({
      ...
      'serverMethod': 'post',
      'ajax': {
        'url':'<?=base_url()?>Myborrowedbooks/fetch_book_borrow/'
      },
      'columns': [
         { data: 'book_name' },
         { data: 'book_grade' },
  ...
  $(document).on('click', '#borrowThisBookName', function() {
    ...
    $.ajax({
      method: "POST",
      url: "<?php echo base_url(); ?>Myborrowedbooks/borrow_book_name/",
      data: ({
        stockid: stockid
      }),
      cache: false,
  ...
  $(document).on('submit', '#submiteBorrowBookName', function(e) {
    ...
    if ($('#dateReturnBook').val() != '') {
      $.ajax({
        method: "POST",
        url: "<?php echo base_url(); ?>Myborrowedbooks/borrowThis_bookName/",
  ...
  $(document).on('click', '.removeBorrowRequest', function() {
    ...
    .then((willDelete) => {
      if (willDelete) {
        ...
        $.ajax({
          method: "POST",
          url: "<?php echo base_url(); ?>Myborrowedbooks/removerequest_borrow_book_name/",
          data: ({
            stockid: stockid
          }),
          cache: false,
