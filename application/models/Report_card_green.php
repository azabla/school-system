...
class reportcard_model extends CI_Model{
            ...
            if($queryCheck->num_rows()>0){
              ...
              foreach($query->result() as $rowLeaving){
                if($rowLeaving->leavingreason!=$reasonName){
                  $output.='<option class="'.$row->username.'" name="'.$rowLeaving->leavingreason.'" value="'.$rowLeaving->id.'" title="'.$academicyear.'">'.$rowLeaving->leavingreason.'</option>';
                }
              }
              $output.='<option class="'.$row->username.'" name="backToDefaultReason" value="backToDefaultReason" title="'.$academicyear.'">Completed Grade '.$row->grade.'</option>';
            }else{
  ...
  function prepareRCTable($max_year){
    ...
    if($queryStudent->num_rows()>0){
      foreach ($queryStudent->result() as $gradesecValue) {
        ...
        if ($queryCheck->num_rows()<1 ){
          ...
          if($query){
            ...
            $output .='Table status active for '.$gradesec.'';
          }/*else{
            $output .='Table already created.';
          }*/
        }/*else{
          $sql = "CREATE INDEX stu_id ON reportcard".$gradesec.$max_year."(stuid,grade,subject,quarter,onreportcard,rpbranch)";
          $querySql=$this->db->query($sql); 
      ...
      $queryCheck3 = $this->db->query("SHOW TABLES LIKE 'semester_card".$max_year."' ");
      if ($queryCheck3->num_rows()<1 ){
        $this->dbforge->add_field($fields);
        $this->dbforge->add_key('rid',TRUE);
        $query=$this->dbforge->create_table('semester_card'.$max_year,TRUE); 
        $sql = "CREATE INDEX stu_id ON semester_card".$max_year."(stuid,grade,subject,quarter,onreportcard,rpbranch)";
        $this->db->query($sql); 
      }/*else{
        $sql = "CREATE INDEX stu_id ON semester_card".$max_year."(stuid,grade,subject,quarter,onreportcard,rpbranch)";
        $this->db->query($sql); 
  ...
  function fetchCustomStudent_quarter($reportaca,$gradesec,$branch,$rpQuarter){
    $this->db->order_by('fname,mname,lname','ASC');
    $this->db->where(array('status'=>'Active'));
    $this->db->where(array('isapproved'=>'1'));
    $this->db->where(array('academicyear'=>$reportaca));
    ...
    foreach ($query ->result() as $value) {
      ...
      <td>'.$value->gradesec.'</td>
      <td>'.$value->branch.' </td> 
      <td><button class="btn btn-info printThisStudentReport_quarter" name="'.$id.'" value="'.$rpQuarter.'" id="'.$reportaca.'"><i class="fas fa-print"></i></button></td> </tr>';
      $no++;
    }
    return $output;
  }
  ...
  function reportcardByQuarter_green($max_year,$gradesec,$branch,$max_quarter){
    $output ='';$resultSem1=0;$printValueSem1=0;$resultSem2=0;
    $resultSem2=0;
    switch ($max_quarter) {
        case 'Quarter1':
          $headerNames="1<sup>st</sup> Quarter";
          break;
        case 'Quarter2':
          $headerNames="1<sup>st</sup> Semester";
          break;
        case 'Quarter3':
          $headerNames="3<sup>rd</sup> Quarter";
          break;
        default:
          $headerNames="";
          break;
    ...
    if($queryCHK->num_rows()>0){
      ...
      if($queryStudent->num_rows()>0){
        ...
        foreach ($queryStudent->result() as $fetchStudent)
        {
          ...
          $username1=$fetchStudent->username;
          //$gradesec=$fetchStudent->gradesec;
          $output.='<div class="support-ticket media card-header">
          ...
          <div class="media-body ml-3">
            ...
            <h4 id="ENScool"><B id="ENS"><u>'.$gYearName.' G.C '.$headerNames.' Student Progress Report </u></B></h4>
          </div>
        </div> ';
          $output.= '<div class="row" id="ENS">
          ...
          and onreportcard='1' and letter='#' or stuid='$stuid' and quarter='Quarter2' and onreportcard='1' and letter='#' ");
            ...
            $printValue2Q3F=0;$printValue2Q3=0;
            $quartrTotal2=$this->db->query("select sum(total) as total from reportcard".$gradesec.$max_year." where stuid='$stuid' and quarter='Quarter3' and onreportcard='1' and letter='#' ");
            foreach ($quartrTotal2->result() as $totalValue2) {
              $printValue2Q3=($totalValue2->total)/$subALl;
              $printValue2Q3F=number_format((float)$printValue2Q3,2,'.','');
            }
          /*3rd and 4th quarter horizontal average calculation starts*/
          $quarterArray2=array('Quarter3','Quarter4');
          foreach ($quarterArray2 as $quarterValuee) {
         ...
         $quartrYATotal=$this->db->query("select sum(total) as total from reportcard".$gradesec.$max_year." where stuid='$stuid' and onreportcard='1' and letter='#' ");
          ...
          if($queryRankAllowed->num_rows()>0){
            ...
            foreach ($quarterArray2 as $quarterValuee) {
              ...
              $query_total=$this->db->query("select * from reportcard".$gradesec.$max_year." where academicyear='$max_year' and stuid ='$stuid' and
              quarter='$quarter' and rpbranch='$branch' group by quarter");

              $query_rank3=$this->db->query("select sum(total),FIND_IN_SET(sum(total), (select GROUP_CONCAT(rank order by rank DESC)from (select sum(total) as rank from 
              reportcard".$gradesec.$max_year." where grade='$gradesec' and letter='#' and quarter='$quarter' and onreportcard='1' and rpbranch='$branch' group by stuid) sm)) as 
              stuRank from reportcard".$gradesec.$max_year." where stuid='$stuid' and quarter= '$quarter' and academicyear='$max_year' and letter='#' and grade='$gradesec' 
              and onreportcard='1' and rpbranch='$branch' group by grade ");
              if($query_total->num_rows()>0){
                foreach ($query_rank3->result() as $qtrank3)
                {
                  
                    $output .= '<td class="text-center" colspan="2"><B>'.$qtrank3->stuRank.'</B></td>';
          ...
          /*2nd semester absent days*/
          $tot2stSem=0;
          $queryTerm=$this->db->query("select startdate,endate from quarter where Academic_year='$max_year' and term='Quarter4' ");
        if($queryTerm->num_rows()>0){
          foreach ($quarterArray3 as $qvalue) {
            $queryTerm=$this->db->query("select startdate,endate from quarter where Academic_year='$max_year' and term='$qvalue' ");
             if($queryTerm->num_rows()>0){
          ...
          $output.='</tr>';
            $output.='</table></div><hr>';
            if($max_quarter=='Quarter2'){
                $query_basicskill=$this->db->query(" Select * from basicskill where grade='$grade' and academicyear='$max_year' and subjectrow='1' and bsname!='Conduct' order by bsname ASC ");   
                if($query_basicskill->num_rows()>0){
                    $output.='<div class="table-responsive">
                    <table width="100%" id="ENS" class="tabler table-borderedr table-md" cellspacing="9" cellpadding="9">';
                    foreach ($query_basicskill->result() as $bsvalue) {
            ...
            switch ($max_quarter) {
                case 'Quarter1':
                  $headerName="1<sup>st</sup> Quarter Homeroom Teacher's Comment";
                  $commentedResult=$printValue;
                  break;
                case 'Quarter2':
                  $headerName="1<sup>st</sup> Semester Homeroom Teacher's Comment";
                  $commentedResult=$printValueSem1;
                  break;
                case 'Quarter3':
                  $headerName="3<sup>rd</sup> Quarter Homeroom Teacher's Comment";
                  $commentedResult=$printValue2Q3F;
                  break;
                default:
                  $headerName="2<sup>nd</sup> Semester Homeroom Teacher's Comment";
                  $commentedResult=$printValueSem2;
                  break;
              }
            $output.="<h4 class='text-success'><b><u>".$headerName."</u></b></h4>";
              ...
              $reportCardComments=$this->db->query("select commentvalue from reportcardcomments where grade='$grade' and '$commentedResult' between mingradevalue and maxgradevalue and academicYear='$max_year'");
              if($reportCardComments->num_rows()>0){
                foreach($reportCardComments->result() as $commentValue){
                  $output .= ''.$commentValue->commentvalue.'<br>';
                }
              }else{
              ...
              $output.='<br>Other Comments ___________________________________________________________________ ___________________________________________________________________  <br>';
              ...
              $queryDi=$this->db->query("select u.fname,u.mname,u.lname ,u.mysign from users as u cross join directorplacement as dp ON dp.staff=u.username where dp.grade='$gradesec' and dp.academicyear='$max_year' and usertype!='Student' and u.branch='$branch' and isapproved='1' and status='Active' and mysign!='' ");
               if($queryDi->num_rows()>0){
                ...
                $output.="Director’s Name:- <u>".$signName." ".$signmame."</u> &nbsp;&nbsp;&nbsp; Signature <img alt='Sig.' src='".base_url()."/".$signSigns."' class='' style='height:40px;width:120px'>";
              }else{
                $output.="Director’s Name______________________ &nbsp;&nbsp;&nbsp;
          ...
          if($queryCategory->num_rows()>0){
            ...
            foreach ($queryCategory->result() as $bscatvalue) {
              $bscategory=$bscatvalue->bscategory;
              $query_basicskill=$this->db->query(" Select * from basicskill where grade='$grade' and academicyear='$max_year' and bsname!='Conduct' and bscategory='$bscategory' group by bsname order by bsname ASC ");       
              $output .='<tr><th colspan="5" id="BGS" class="text-center">'.$bscategory.'</th>';
              foreach ($query_basicskill->result() as $bsvalue) {
                $bsname=$bsvalue->bsname;
                $output .='<tr><td>'.$bsvalue->bsname.'</td>';
  ...
  function transcriptSeattle($academicyear,$gradesec,$branch,$noGrade)
  {
    $output='';
  ...
  function letterTranscriptSeattle($academicyear,$gradesec,$branch,$noGrade)
  {
    $output='';
    $targetYear=$academicyear;
    $queryUsers=$this->db->query("select grade,username,unique_id from users where gradesec='$gradesec' and usertype='Student' and branch='$branch' and status='Active' and isapproved='1' and academicyear='$academicyear' group by grade order by grade ASC ");
  ...
  function transcriptCustomSeattle($academicyear,$gradesec,$branch,$userName,$noGrade)
  {
    $output='';
    $targetYear=$academicyear;
  ...
  function letterCustomTranscriptSeattle($academicyear,$gradesec,$branch,$userName,$noGrade)
  {
    $output='';
    $targetYear=$academicyear;
    $queryUsers=$this->db->query("select fname,mname,lname,gender,gradesec,age,username, unique_id, grade, id, profile from users where gradesec='$gradesec' and usertype='Student' and branch='$branch' and status='Active' and isapproved='1' and academicyear='$academicyear' and username='$userName' order by fname,mname,lname ASC ");
  ...
  function reportcardGreen($max_year,$gradesec,$branch,$max_quarter,$includeBackPage){
    $output ='';
    $queryCHK=$this->db->query("select * from reportcard".$gradesec.$max_year." where grade='$gradesec' and academicyear='$max_year' and rpbranch='$branch' and onreportcard='1' group by subject order by subjorder ASC");
    if($queryCHK->num_rows()>0){
      ...
      if($queryStudent->num_rows()>0){
        ...
        foreach ($queryStudent->result() as $fetchStudent)
        {
            $result2=0;$result1=0;$resultSem1=0;$resultSem2=0;
          $grade=$fetchStudent->grade;
          $stuid=$fetchStudent->id;
          $section=$fetchStudent->section;
          $username1=$fetchStudent->username;
          ...
          $output.='<tr><th colspan="4" class="text-center">STUDENT GRADE REPORT</th></tr>
          <tr><td class="text-center"><h3> Subjects</h3></td>
          <td class="text-center">1st Semester<br>100%</td>
          <td class="text-center">2nd Semester<br>100%</td>
          <td class="text-center">Yearly Average</td> </tr>';
          ...
          $output.='</tr>';
            ...
            $output.= '<div class="col-lg-6 col-md-6">';
            $output.= '<p class="text-center">Grade:-<b> '.$grade.' &nbsp;&nbsp; Section:- '.$section.'</b></p>
            <div id="ENS" class="table-responsive">
            <table width="100%" id="ENS" class="tabler table-borderedr table-md" cellspacing="5" cellpadding="5">';
            $output .='<tr><th class="text-center" colspan="3">GRADING SCALE</th></tr>';
            $output .='<tr><td class="text-center">% Percent</td><td class="text-center">Grade</td><td class="text-center">Description</td></tr>';
            $output .='<tr><td class="text-center">90 to 100</td> <td class="text-center">A</td><td class="text-center">Excellent</td></tr>';
            $output .='<tr><td class="text-center">80 to 89</td> <td class="text-center">B</td><td class="text-center">Very Good</td></tr>';
            $output .='<tr><td class="text-center">70 to 79</td> <td class="text-center">C</td><td class="text-center">Good</td></tr>';
            $output .='<tr><td class="text-center">60 to 69</td> <td class="text-center">D</td><td class="text-center">Needs Improvement</td></tr>';
            $output .='<tr><td class="text-center">Below 59 </td> <td class="text-center">F</td><td class="text-center">Unsatisfactory</td></tr>';
            ...
            if($queryCategory->num_rows()>0){
              foreach ($queryCategory->result() as $bscatvalue) {
                ...
                $no=1;
                $output .='<tr>
                <th rowspan="2">'.$bscategory.'</th><th colspan="2" class="text-center">Semester</th></tr><tr>';
                ...
                foreach ($quarterArrayQ as $qvalue) 
                {
                  ...
                  $query_basicskill=$this->db->query(" Select * from basicskill where grade='$grade' and academicyear='$max_year' and bscategory='$bscategory' and subjectrow='0' order by bsname ASC ");    
                  $output .='<tr>';
                  foreach ($query_basicskill->result() as $bsvalue) {
                    $bsname=$bsvalue->bsname;
                    $output .='<tr><td>'.$bsvalue->bsname.'</td>';
                ...
                $output .='No basic skill found<br>';
            }
            $queryEvaKey=$this->db->query("select * from bstype where academicyear='$max_year' and btgrade='$grade' ");
            if($queryEvaKey->num_rows()>0){
                $output.= '<div class="row">';
                $output .='<div class="col-lg-12 col-12 text-center" id="BGS"><h4 style="font-family:century_gothicbold;">Desirable traits & habits of the student</h4></div>';
                foreach ($queryEvaKey->result() as $keyVValue) {
                    $output .='<div class="col-lg-6 col-12"><h5 style="font-family:century_gothicbold;">'.$keyVValue->bstype.' => '.$keyVValue->bsdesc.'</h5></div>';
                }
                $output .='</div>';/*Evaluation key table closed*/
            ...
            $output.='<p class="reportcardView">Note: Erased, Altered or Unsealed report card is invalid.</p>';
          ...
          $query_name = $this->db->query("select * from school");
            ...
            $email=$row_name->email;
            $logo=$row_name->logo;
            $output.='<h5 class="text-center text-success"><b><u>Homeroom Teacher Comments & Recommendations</u></b></h5>
                  <div class="row">
                    <div class="col-lg-12 col-12 reportcardView">';
                    $queryHoomRoom=$this->db->query("select u.fname,u.mname,u.mysign from users as u cross join hoomroomplacement as hm where hm.roomgrade='$gradesec' and hm.academicyear='$max_year' and hm.branch='$branch' and u.branch='$branch' and u.username=hm.teacher and u.status='Active' and u.isapproved='1' ");
                    if($queryHoomRoom->num_rows()>0){
                      $rowHommeRoom=$queryHoomRoom->row_array();
                      $tfName=$rowHommeRoom['fname'];
                      $tmName=$rowHommeRoom['mname'];
                      $signSignsD=$rowHommeRoom['mysign'];
                    }else{
                      $tfName='------';
                      $tmName='------';
                      $signSignsD='_____';
                    ...
                    $subALl=$countSubject->num_rows();
                      if($subALl>0){
                        ...
                        $output.=" <p>HRT's Name: <u><b>".$tfName." ".$tmName."</b></u> Sig.<img alt='.' src='".base_url()."/".$signSignsD."' style='height:40px;width:120px'>________________________ </p>
                       Parent's Name: <u><b>".$mname." " .$lname." </b></u> Sig.________________________
                    </div>";
                    $output.='<div class="col-lg-12 col-12 reportcardView">';
                        ...
                        $output.=" <p>HRT's Name: <u><b>".$tfName." ".$tmName."</b></u> Sig.________________________ </p>
                       Parent's Name: <u><b>".$mname." " .$lname." </b></u> Sig.________________________ 
                    </div>";
              ...
              <div class="col-lg-6 col-6">
                <div class="row">
                  <div class="col-lg-12 col-12">';
                  $output.='<h1 class="text-center text-success">ግሪን ት/ቤት <br> GREEN NEW GENERATION SCHOOL</h1>
                  <div class="row">
                  <div class="col-lg-6 col-6 text-success"><h4>
                  ከአፀደ ህፃናት - 2ኛ ደረጃ<br>
                  `የልጆችን ነገ እንፈጥራለን::`</h4>
                  </div>
                  <div class="col-lg-6 col-6 text-success"><h4>KG - Preparatory<br>
                  Creating Childrens Future</h4></div>
                  </div>
                  ...
                  <div class="col-lg-12 col-12">
                    <p class="text-center"><i class="fas fa-phone-square"></i>: 0118889423/+2519-29-90-5405<br>
                    <i class="fas fa-envelope"></i> '.$email.'<br>Website address- www.greenschoolethiopia.com<br>
  ...
  function customReportCardGreen($max_year,$gradesec,$branch,$rpQuarter,$id,$includeBackPage){
    $output ='';
    $queryCHK=$this->db->query("select * from reportcard".$gradesec.$max_year." where grade='$gradesec' and academicyear='$max_year' and rpbranch='$branch' and onreportcard='1' group by subject order by subjorder ASC");
    if($queryCHK->num_rows()>0){
      /*$queryStudent=$this->db->query(" Select fname,mname,lname,id,grade,gender,age,city,kebele, profile, section,gradesec,username,woreda from users where gradesec='$gradesec' and usertype='Student' and branch='$branch' and isapproved='1' and status='Active' and academicyear='$max_year' order by fname,mname,lname ASC ");
      ...
      if($queryStudent->num_rows()>0){
        ...
        foreach ($queryStudent->result() as $fetchStudent)
        {
          ...
          $output.='</tr>';
            ...
            <div id="ENS" class="table-responsive">
            <table width="100%" id="ENS" class="tabler table-borderedr table-md" cellspacing="5" cellpadding="5">';
            $output .='<tr><th class="text-center text-success" colspan="3">GRADING SCALE</th></tr>';
            $output .='<tr><td class="text-center">% Percent</td><td class="text-center">Grade</td><td class="text-center">Description</td></tr>';
            $output .='<tr><td class="text-center">90 to 100</td> <td class="text-center">A</td><td class="text-center">Excellent</td></tr>';
            $output .='<tr><td class="text-center">80 to 89</td> <td class="text-center">B</td><td class="text-center">Very Good</td></tr>';
            $output .='<tr><td class="text-center">70 to 79</td> <td class="text-center">C</td><td class="text-center">Good</td></tr>';
          ...
          $query_name = $this->db->query("select * from school");
            ...
            $output.='<h5 class="text-center text-success"><b><u>Homeroom Teacher Comments & Recommendations</u></b></h5>
                  <div class="row">
                    <div class="col-lg-12 col-12 reportcardView">';
                    /*$queryHoomRoom=$this->db->query("select u.fname,u.mname,mysign from users as u cross join 
                    hoomroomplacement as hm where hm.roomgrade='$gradesec' and hm.academicyear='$max_year' and hm.branch='$branch' and u.branch='$branch' and 
                    u.username=hm.teacher and u.status='Active' and u.isapproved='1' ");
                  if($queryHoomRoom->num_rows()>0){
                    $rowHommeRoom=$queryHoomRoom->row_array();
                    $tfName=$rowHommeRoom['fname'];
                    $tmName=$rowHommeRoom['mname'];
                    ...
                    $queryHoomRoom=$this->db->query("select u.fname,u.mname,mysign from users as u cross join hoomroomplacement as hm where hm.roomgrade='$gradesec' and 
                    hm.academicyear='$max_year' and hm.branch='$branch' and u.branch='$branch' and u.username=hm.teacher and u.status='Active' and u.isapproved='1' ");
                    if($queryHoomRoom->num_rows()>0){
                      $rowHommeRoom=$queryHoomRoom->row_array();
                      $tfName=$rowHommeRoom['fname'];
                      $tmName=$rowHommeRoom['mname'];
                    ...
                    $subALl=$countSubject->num_rows();
                      if($subALl>0){
                        ...
                        $output.=" <p>HRT's Name: <u><b>".$tfName." ".$tmName."</b></u> Sig.<img alt='.' src='".base_url()."/".$signSigns."' style='height:40px;width:120px'>________________________ </p>
                       Parent's Name: <u><b>".$mname." " .$lname." </b></u> Sig.________________________
                    </div>";
                    $output.='<div class="col-lg-12 col-12 reportcardView">';
                        if($subALl>0){
  ...
  function reportcardKg_Green($max_year,$gradesec,$branch,$max_quarter,$includeBackPage){
    $output ='';
    $queryCHK=$this->db->query("select * from reportcard".$gradesec.$max_year." where grade='$gradesec' and academicyear='$max_year' and rpbranch='$branch' and onreportcard='1' group by subject order by subjorder ASC");
    if($queryCHK->num_rows()>0){
      $queryStudent=$this->db->query(" Select fname,mname,lname,id,grade,gender,age,city,kebele, profile, section,gradesec,username,woreda from users where gradesec='$gradesec' and usertype='Student' and branch='$branch' and isapproved='1' and status='Active' and academicyear='$max_year' order by fname,mname,lname ASC ");
      ...
      if($queryStudent->num_rows()>0){
        ...
        foreach ($queryStudent->result() as $fetchStudent)
        {
          ...
          $quarterArrayQ1=array('semester1','semester2');
            ...
            if($query_basicskill->num_rows()>0){  
              ...
              foreach ($query_basicskill->result() as $bsvalue) {
                ...
                $output .='<td><B>'.$bsvalue->bsname.'</B></td>';
                foreach ($quarterArrayQ1 as $qvalue) {
                  $query_bsvalue=$this->db->query(" Select * from basicskillvalue".$gradesec.$max_year." where stuid='$stuid' and academicyear='$max_year' and bsname='$bsname' 
                  and quarter='$qvalue' ");
                  if($query_bsvalue->num_rows()>0) {
                    foreach ($query_bsvalue ->result() as $bsresult) {
                      $output .='<td class="text-center">'.$bsresult->value.'</td>';
          ...
          $output.='</tr>';
            ...
            $output.= '<div class="col-lg-6 col-md-6">';
            $output.= '<p class="text-center">Grade:-<b> '.$grade.' &nbsp;&nbsp; Section:- '.$section.'</b></p>
            <span class="text-center">The values of grades given to the student’s academic performance are given below:</span>
            <div id="ENS" class="table-responsive">
            <table width="100%" id="ENS" class="tabler table-borderedr table-md" cellspacing="5" cellpadding="5">';
            $output .='<tr><th class="text-center text-success" colspan="3">GRADING SYSTEM</th></tr>';
            $output .='<tr><td class="text-center">Grade</td><td class="text-center">Performance Levels</td><td class="text-center">Marks Range</td></tr>';
            $output .='<tr><td class="text-center">A+</td> <td class="text-center">Excellent</td><td class="text-center">95 - 100</td></tr>';
            $output .='<tr><td class="text-center">A</td> <td class="text-center">superfine</td><td class="text-center">90 - 94</td></tr>';
            $output .='<tr><td class="text-center">B+</td> <td class="text-center">Very Good</td><td class="text-center">85 - 89</td></tr>';
            $output .='<tr><td class="text-center">B</td> <td class="text-center">Fine</td><td class="text-center">80 - 84</td></tr>';
            $output .='<tr><td class="text-center">C</td> <td class="text-center">Good</td><td class="text-center">70 - 80</td></tr>';
            $output .='<tr><td class="text-center">NI</td> <td class="text-center">Needs Improvement</td><td class="text-center"><70</td></tr>';
            $output.= '</table></div><br><br>';
            $query_name = $this->db->query("select * from school");
            $row_name = $query_name->row();
            $school_name=$row_name->name;
            ...
            $output.='<h5 class="text-center text-info"><b><u>Homeroom Teacher Comments & Recommendations</u></b></h5>
                  <div class="row">
                    ...
                    $countSubject=$this->db->query("select * from reportcard".$gradesec.$max_year." where grade='$gradesec' and academicyear='$max_year' 
                    and rpbranch='$branch' and onreportcard='1' group by subject order by subjorder ASC");
                    $subALl=$countSubject->num_rows();
                      if($subALl>0){
                        $output.="<p class='text-center'><b>1<sup>st</sup> Semester</b></p>";
                        $quartrTotal=$this->db->query("select sum(total) as total from reportcard".$gradesec.$max_year." where stuid='$stuid' and quarter='semester1' and onreportcard='1' ");
                        if($quartrTotal->num_rows()>0){
                          foreach ($quartrTotal->result() as $totalValue) {
                            $printValue1=($totalValue->total)/$subALl;
                    ...
                    $output.='<div class="col-lg-12 col-12 reportcardView">';
                        if($subALl>0){
                        $output.="<p class='text-center'><b>2<sup>nd</sup> Semester</b></p>";
                        $quartrTotal=$this->db->query("select sum(total) as total from reportcard".$gradesec.$max_year." where stuid='$stuid' and quarter='semester2' and onreportcard='1' ");
                        if($quartrTotal->num_rows()>0){
                          foreach ($quartrTotal->result() as $totalValue) {
                            $printValue2=($totalValue->total)/$subALl;
                            $reportCardComments=$this->db->query("select commentvalue from reportcardcomments where grade='$grade' and $printValue2 between mingradevalue and maxgradevalue and academicYear='$max_year'");
          ...
          if($includeBackPage=='1')
          {
            ...
            if($queryCategory->num_rows()>0){
              foreach ($queryCategory->result() as $bscatvalue) {
                $bscategory=$bscatvalue->bscategory;
                $output.= '<h4 class="text-center text-success"><u>Student’s behavioural & other activities performance</u></h4>
                <div class="table-responsive">
                <table width="100%" id="ENS" class="tabler table-bordered table-md" cellspacing="5" cellpadding="5">';
                $dateYear=date('Y');
                $no=1;
            ...
            $output.= "<span>* Include Parent's effort and participation.</span><br> ";
            $output.= '<span class="text-center">The values of grades given to the student’s behavioural performance are given below:</span>
            <div id="ENS" class="table-responsive">
            <table width="100%" id="ENS" class="tabler table-borderedr table-md" cellspacing="5" cellpadding="5">';
            $output .='<tr><th class="text-center text-success" colspan="3">GRADING SYSTEM</th></tr>';
            $output .='<tr><td class="text-center">Performance Levels</td><td class="text-center">Description</td></tr>';
            $output .='<tr> <td class="text-center">E</td><td class="text-center">Excellent</td></tr>';
            $output .='<tr> <td class="text-center">VG</td><td class="text-center">Very Good</td></tr>';
            $output .='<tr> <td class="text-center">G</td><td class="text-center">Good</td></tr>';
            $output .='<tr> <td class="text-center">NI</td><td class="text-center">Needs Improvement</td></tr>';
            $output.= '</table></div><br><br>';
  ...
  function quarterosterGreen($max_year,$gradesec,$branch,$quarter){
    $query_student=$this->db->query(" Select * from users where gradesec='$gradesec' and branch='$branch' and status='Active' and academicyear='$max_year' and isapproved='1' and usertype='Student' group by id order by fname,mname,lname ASC ");
    $query_name = $this->db->query("select * from school");
    $row_name = $query_name->row();//school info
    $school_name=$row_name->name;
  ...
  function rosterGreen($max_year,$gradesec,$branch,$page)
  {
    $query_student=$this->db->query(" Select * from users where gradesec='$gradesec' and branch='$branch' and status='Active' and academicyear='$max_year' and isapproved='1' and usertype='Student' group by id order by fname,mname,lname ASC ");
    $query_name = $this->db->query("select * from school");
    $row_name = $query_name->row();//school info
    ...
    foreach ($query_student->result() as $row) 
    {
      ...
      if($row->age==0){
          $output.='<td rowspan="4">__</td>';
      }else{
        $output.='<td rowspan="4">'.$row->age.'</td>';  
      }
      $output.='<td rowspan="4">'.$row->gender.'</td>';
      foreach ($query_quarter as $qvalue) 
      {
        $quarter=$qvalue;
        $output .='<tr> <td>'.$qvalue.'</td>';
    ...
    if($queryCheck->num_rows()>0 ){
      ...
      if($queryStudent->num_rows()>0){
        ...
        foreach ($queryStudent->result() as $fetchStudent)
        {
          ...
          $woreda=$fetchStudent->woreda;
          $profile=$fetchStudent->profile;
          $output.= '<div style="width:100%;height:auto;page-break-inside:avoid;display: block; "> <div class="row" id="ENS">
          <div class="col-lg-7 col-md-7">';
          $output.='<h4 class="text-info"><b>
          Student Name:- '.ucfirst(strtolower($fname)).' '.ucfirst(strtolower($mname)).' '.ucfirst(strtolower($lname)).'</b></h4>';
           $output.='<div class="table-responsive">
  ...
  function Fetch_student_semester_roster($max_year,$gradesec,$branch,$page){
    $query_student=$this->db->query(" Select * from users where gradesec='$gradesec' and branch='$branch' and status='Active' and academicyear='$max_year' and isapproved='1' and usertype='Student' group by id order by fname,mname,lname ASC ");
      $stuAll=$query_student->num_rows();

    $semester_card=array();
    foreach ($query_student->result() as $fetchStudent)
    {
      $result1=0;$result2=0;
      $grade=$fetchStudent->grade;
      $stuid=$fetchStudent->id;
      ...
      $lname=$fetchStudent->lname;
      $this->db->where('academicyear',$max_year);
      $this->db->where('grade',$grade_sec);
      $this->db->where('rpbranch',$branch);
      $this->db->delete('semester_card'.$max_year);
      $querySubject=$this->db->query("select * from reportcard".$gradesec.$max_year." where grade='$gradesec' and academicyear='$max_year' and rpbranch='$branch' and onreportcard='1' group by subject order by subjorder ASC");
      if($querySubject->num_rows()>0){
        $finalresult=0;
        foreach ($querySubject->result() as $fetchSubject) {
          $subject=$fetchSubject->subject;
          $letter=$fetchSubject->letter;
          $mergedname=$fetchSubject->mergedname;
          $onreportcard=$fetchSubject->onreportcard;
          $subjorder=$fetchSubject->subjorder;
          $queryTerm2=$this->db->query("select * from reportcard".$gradesec.$max_year." where rpbranch='$branch' and stuid='$stuid' and quarter='Term2' and subject='$subject' order by subjorder");
          $fResultTerm2=0;$resultTerm_1=0;
          if($queryTerm2->num_rows()>0){
            foreach ($queryTerm2->result() as $fetchTerm2Result) {
              $resultTerm2=($fetchTerm2Result->total)/2;
              $fResultTerm2=number_format((float)$resultTerm2,2,'.','');
            }
          }
          $arrayTerm=array('Term1','Term3');
          foreach($arrayTerm as $terms){
            $queryReportCardQ1=$this->db->query("select * from reportcard".$gradesec.$max_year." where rpbranch='$branch' and stuid='$stuid' and quarter='$terms' and subject='$subject' order by subjorder ");
            if($queryReportCardQ1->num_rows()>0){
              foreach ($queryReportCardQ1->result() as $fetchResult1) {
                $resultTerm_1=($fetchResult1->total) + $fResultTerm2;
                $result1=($resultTerm_1 * 100)/150;
                $finalresult =number_format((float)$result1,2,'.','');
              }
            }
            $semester_card[]=array(
              'stuid'=>$stuid,
        ...
        $queryInsert=$this->db->insert_batch('semester_card'.$max_year,$semester_card);
      }
    }
    $query_name = $this->db->query("select * from school");
    $row_name = $query_name->row();//school info
    ...
    foreach ($query_student->result() as $row) 
    {
      ...
      $grade_sec=$row->gradesec;
      $grade=$row->grade;
      $query_quarter=array('Term1','Term3');
      $output .='<tr><td rowspan="4">'.$stuNo.'.</td>
      <td rowspan="4">'.$row->fname.' '.$row->mname.' '.$row->lname.'</td> 
      ...
      foreach ($query_quarter as $qvalue) 
      {
        ...
        switch ($quarter) {
          case 'Term1':
            $headerName="1<sup>st</sup> Semester";
            break;
          case 'Term3':
            $headerName="2<sup>nd</sup> Semester";
            break;
          default:
            $headerName="2<sup>nd</sup> Semester";
            break;
        }
        $output .='<tr> <td>'.$headerName.'</td>';
        foreach ($query_result ->result() as $subname)
        {
          $subject_mark=$subname->subject;
          $queryTerm2=$this->db->query("select * from reportcard".$gradesec.$max_year." where rpbranch='$branch' and stuid='$stuid' and quarter='Term2' and subject='$subject_mark' order by subjorder");
          $fResultTerm2=0;$resultTerm_1=0;
          if($queryTerm2->num_rows()>0){
            foreach ($queryTerm2->result() as $fetchTerm2Result) {
              $resultTerm2=($fetchTerm2Result->total)/2;
          ...
          if($query_quarter_result->num_rows()>0){
            foreach ($query_quarter_result->result() as $quarter_result) 
            {
                $letter=$quarter_result->letter;
                $result=$quarter_result->total;
               $resultTerm_1=($quarter_result->total) + $fResultTerm2;
                $result1=number_format((float)($resultTerm_1 * 100)/150,2,'.','');
              if($result1>0){
                  if($letter!='A')
                  {
                    $output .='<td class="text-center">'.$result1.'</td>';
                  }
                  else{
                    $queryRange=$this->db->query("select letterVal from letterange where grade='$grade' and $result1 between minValue and maxiValue and academicYear='$max_year'");
                    if($queryRange->num_rows()>0){
        ...
        $query_quarter_total=$this->db->query(" Select sum(total) as quarter_total from semester_card".$max_year." where grade='$gradesec' and rpbranch='$branch' and stuid='$stuid' and academicyear='$max_year' and letter!='A' and quarter ='$quarter' and onreportcard='1' and mergedname='' group by quarter order by subject ASC ");
        $count_subject=$this->db->query("select * from semester_card".$max_year." where grade='$gradesec' and rpbranch='$branch' and letter!='A' and onreportcard='1' and mergedname='' and academicyear='$max_year' group by subject ");
        $total_subject=$count_subject->num_rows();
        if($query_quarter_total->num_rows()>0){
          foreach ($query_quarter_total->result() as $qtvalue)
          {
          ...
          $query_rank=$this->db->query("select sum(total),FIND_IN_SET(sum(total), (select GROUP_CONCAT(rank order by rank DESC)from (select sum(total) as rank from semester_card".$max_year." where grade='$gradesec' and rpbranch='$branch' and letter='#' and onreportcard='1' and mergedname='' and quarter='$quarter' group by stuid) sm)) as stuRank from semester_card".$max_year." where stuid='$stuid' and quarter= '$quarter' and rpbranch='$branch' and academicyear='$max_year' and letter='#' and onreportcard='1' and mergedname='' and grade='$gradesec' group by grade ");
          foreach ($query_rank->result() as $rvalue) {
            $output .= '<td class="text-center">'.$rvalue->stuRank.'</td>';//Each quarter rank
          }
        }else{
      ...
      /*Yearly Average calculation starts*/
      $output .='<tr id="BGS"><td>Yearly Average </td>';
      $queryCheckSem2=$this->db->query("select *, sum(total) as total from semester_card".$max_year." where stuid='$stuid' and quarter='Term3' ");
      foreach ($query_result ->result() as $subname)
      {
        $subject_mark=$subname->subject;
      ...
      $quartrSem2Total=$this->db->query("select sum(total) as total from semester_card".$max_year." where stuid='$stuid' and onreportcard='1' and letter='#' ");
      if($queryCheckSem2->num_rows()>0){
        foreach ($quartrSem2Total->result() as $totalValueSem2) {
          $printValueSem2=(($totalValueSem2->total)/2);
          if($printValueSem2 >0){
          ...
          if($queryReportCardQ4->num_rows()>0){
            ...
            switch ($max_quarter) {
                ...
                case 'Quarter3':
                  ...
                  break;
                default:
                  $headerName="Annual Homeroom Teacher's Comment";
                  $commentedResult=$printValueSem2;
                  break;
              }
            $output.="<h4 class='text-success'><b><u>".$headerName."</u></b></h4>";
