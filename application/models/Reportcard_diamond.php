...
class reportcard_model extends CI_Model{
  ...
  function customReportCard($max_year,$gradesec,$branch,$max_quarter,$id,$includeBackPage){
    $output ='';
    $resultSem1=0;$resultSem2=0;
    ...
    if($queryCHK->num_rows()>0){
      ...
      if($queryStudent->num_rows()>0){
        ...
        foreach ($queryStudent->result() as $fetchStudent)
        {
          ...
          /*if($queryReportCardQ4->num_rows()>0){
            $queryTotalAbsent=$this->db->query("select count(stuid) as att from attendance as attt cross join quarter as se where attt.stuid='$username1' 
            and attt.absentdate between se.startdate and se.endate and se.term='Quarter3' and attt.academicyear='$max_year' and se.Academic_year='$max_year'
            or attt.stuid='$username1' and attt.absentdate between se.startdate and se.endate and se.term='Quarter4' and attt.academicyear='$max_year' 
            and se.Academic_year='$max_year' ");
            if($queryTotalAbsent->num_rows()>0){
              foreach ($queryTotalAbsent->result() as $absent){
                if($absent->att>0)
                {
  ...
  function reportcardByQuarter($max_year,$gradesec,$branch,$max_quarter,$includeBackPage){
    $output ='';
    $resultSem1=0;
    $resultSem2=0;
    $queryCHK=$this->db->query("select * from reportcard".$gradesec.$max_year." where grade='$gradesec' and academicyear='$max_year' and rpbranch='$branch' and onreportcard='1' group by subject order by subjorder ASC");
    if($queryCHK->num_rows()>0){
      $queryStudent=$this->db->query(" Select fname,mname,lname,id,grade,age, gender, profile, city, kebele, section,gradesec, username from users where gradesec='$gradesec' and usertype='Student' and branch='$branch' and isapproved='1' and status='Active' and academicyear='$max_year' order by fname,mname,lname ASC ");
      $stuAll=$queryStudent->num_rows();
      if($queryStudent->num_rows()>0){
        $query_name = $this->db->query("select * from school");
        $row_name = $query_name->row();
        ...
        foreach ($queryStudent->result() as $fetchStudent)
        {
         ...
         $quartrYATotal=$this->db->query("select sum(total) as total from reportcard".$gradesec.$max_year." where stuid='$stuid' and onreportcard='1' and letter='#' ");
          ...
          $output .='</div><br>';/*basic skill column closed*/
          $output.='</div>';/*class row closed*/
          $output.='<div class="dropdown-divider2"></div><h6 class="text-center"><strong>You can always do better than this!</strong></h6></div>';
        }
      } 
    }
    return $output;
  ...
  function rosterDiamond($max_year,$gradesec,$branch,$page)
  {
    $query_student=$this->db->query(" Select * from users where gradesec='$gradesec' and branch='$branch' and status='Active' and academicyear='$max_year' and isapproved='1' and usertype='Student' group by id order by fname,mname,lname ASC ");
    $query_name = $this->db->query("select * from school");
    $row_name = $query_name->row();//school info
    ...
    foreach ($query_student->result() as $row) 
    {
      ...
      $grade_sec=$row->gradesec;
      $grade=$row->grade;
      $query_quarter=array('Term1','Term2','Term3');
      $output .='<tr><td rowspan="5">'.$stuNo.'.</td>
      <td rowspan="5">'.$row->fname.' '.$row->mname.' '.$row->lname.'</td> 
      <td rowspan="5">'.$row->age.'</td>
      ...
      $queryCheckSem2=$this->db->query("select *, sum(total) as total from reportcard".$gradesec.$max_year." where stuid='$stuid' and subject='$subject_mark' and quarter='Term3' ");
      foreach ($query_result ->result() as $subname)
      {
        $subject_mark=$subname->subject;
      ...
      if($queryCheckSem2->num_rows()>0){
        foreach ($quartrSem2Total->result() as $totalValueSem2) {
          $printValueSem2=(($totalValueSem2->total)/3);
          if($printValueSem2 >0){
      ...
      if($queryCheckSem2->num_rows()>0 && $total_subject>0){
        foreach ($quartrSem2Total->result() as $totalValueSem2) {
          $printValueSem2=(($totalValueSem2->total)/3)/$total_subject;
          if($printValueSem2 >0){
            $output .= '<td class="text-center">'.number_format((float)$printValueSem2,2,'.','').'</td>';
          }else{
            $output.='<td class="text-center">-</td>';
  ...
  function reportcardDiamond($max_year,$gradesec,$branch,$max_quarter,$includeBackPage){
    $output ='';
    $resultSem1=0;
    $resultSem2=0;
    $queryCHK=$this->db->query("select * from reportcard".$gradesec.$max_year." where grade='$gradesec' and academicyear='$max_year' and rpbranch='$branch' and onreportcard='1' group by subject order by subjorder ASC");
    if($queryCHK->num_rows()>0){
      ...
      if($queryStudent->num_rows()>0){
        ...
        foreach ($queryStudent->result() as $fetchStudent)
        {
          ...
          $output.='<div class="table-responsive">
          <table width="100%" id="ENS" class="tabler table-borderedr table-md" cellspacing="9" cellpadding="9">';
          $output.='<tr><th colspan="5" class="text-center">
          <h6 id="ENScool"><B id="ENS">'.$school_name.' '.$gYearName.' G.C ('.$max_year.' E.C) Student Report Card</B></h6>
          </th></tr>
          <tr><th class="text-center">Subject</th>
          <th class="text-center">1<sup>st</sup> Term</th>
          <th class="text-center">2<sup>nd</sup> Term</th>
          <th class="text-center">3<sup>rd</sup> Term</th>
          ...
          if($querySubject->num_rows()>0){
            foreach ($querySubject->result() as $fetchSubject) {
              ...
              $letter=$fetchSubject->letter;
              $output.='<tr><td style="white-space: nowrap"><B>'.$fetchSubject->subject.'</B></td>';/*Subject List*/
              $queryReportCardQ1=$this->db->query("select * from reportcard".$gradesec.$max_year." where rpbranch='$branch' and stuid='$stuid' and quarter='Term1' and subject='$subject' order by subjorder ");
              if($queryReportCardQ1->num_rows()>0){
                foreach ($queryReportCardQ1->result() as $fetchResult1) {
                  $result1=$fetchResult1->total;
                  if($fetchResult1->total=='' || $fetchResult1->total<=0){
               ...
               /*fetch quarter 2 result starts*/
              $queryReportCardQ2=$this->db->query("select * from reportcard".$gradesec.$max_year." where rpbranch='$branch' and stuid='$stuid' and quarter='Term2' and subject='$subject' order by subjorder ");
              if($queryReportCardQ2->num_rows()>0){
                foreach ($queryReportCardQ2->result() as $fetchResult2) {
                  $result2=$fetchResult2->total;
                  if($fetchResult2->total=='' || $fetchResult2->total<=0){
              ...
              /*fetch quarter 3 result starts*/
              $queryReportCardQ3=$this->db->query("select * from reportcard".$gradesec.$max_year." where rpbranch='$branch' and stuid='$stuid' and quarter='Term3' and subject='$subject' order by subjorder ");
              if($queryReportCardQ3->num_rows()>0){
                foreach ($queryReportCardQ3->result() as $fetchResult3) {
                  $result3=$fetchResult3->total;
                  if($fetchResult3->total=='' || $fetchResult3->total<=0){
          ...
          $quarterArrayQ=array('Term1','Term2','Term3');
            $query_basicskill=$this->db->query(" Select * from basicskill where grade='$grade' and academicyear='$max_year' and subjectrow='1' order by bsname ASC ");       
            foreach ($query_basicskill->result() as $bsvalue) {
              $bsname=$bsvalue->bsname;
          ...
          if($queryTotalAllowed->num_rows()>0){
            $output.='<tr><td><b>Total</b></td>';
            $quarterArray1=array('Term1','Term2','Term3');
            foreach ($quarterArray1 as $quarterValue) {
              $quartrTotal=$this->db->query("select sum(total) as total from reportcard".$gradesec.$max_year." where stuid='$stuid' and quarter='$quarterValue' and onreportcard='1' and letter='#' ");
              if($quartrTotal->num_rows()>0){
                foreach ($quartrTotal->result() as $totalValue) {
            ...
            $quartrYATotal=$this->db->query("select sum(total) as total from reportcard".$gradesec.$max_year." where stuid='$stuid' and onreportcard='1' and letter='#' ");
              if($quartrYATotal->num_rows()>0){
                  if($resultSem2 ==0 || $resultSem1 == 0){
                    ...
                    foreach ($quartrYATotal->result() as $totalValueYA) {
                      $printValueYA=($totalValueYA->total)/3;
                      if($printValueYA >0){
           ...
           $quartrYATotal=$this->db->query("select sum(total) as total from reportcard".$gradesec.$max_year." where stuid='$stuid' and onreportcard='1' and letter='#' ");
              if($quartrYATotal->num_rows()>0){
                  if($resultSem2 ==0 || $resultSem1 == 0){
                    ...
                    foreach ($quartrYATotal->result() as $totalValueYA) {
                      $printValueYA=(($totalValueYA->total)/3)/$subALl;
                      if($printValueYA >0){
                        $output .= '<td class="text-center"><b>'.number_format((float)$printValueYA,2,'.','').'</b></td>';
                      }else{
                        $output.='<td class="text-center">-</td>';
          ...
          <div class='row StudentViewTextInfo'>
            <div class ='col-md-6 col-6'>
              <p><u>Evaluation Keys</u></p>
              <p>85 - 100 = A</p>
              <p>70 - 84 = B</p>
              <p>60 - 69 = C</p>
              <p>50 - 59 = D</p>
              <p>Below 50%  = NSI</p>
              <p>NI = Not Applicable</p>
            </div>
            <div class ='col-md-6 col-6'>
             <p><u>Category</u></p>
              <p>A = Excellent</p>
              ...
              <p>C = Good</p>
              <p>D = Satisfactory</p>
              <p>NSI = Needs Serious Improvement</p>
            </div>
          </div>
          </div>";/*result table closed*/

          $output.= '<div class="col-lg-6 col-md-6 StudentViewTextInfo">';
          ...
          $query_name = $this->db->query("select * from school");
            ...
            <div class="row" id="ENS">
              <div class="col-lg-12 col-12">
                <div class="row">
                  <div class="col-lg-9 col-12">
                    <div class="row">
                      <div class="col-lg-12 col-12">
                        <p><b id="ENS">Name of the Student: <u>'.ucfirst(strtolower($fname)).' '.ucfirst(strtolower($mname)).' '.ucfirst(strtolower($lname)).'</u></b></p>
                      </div>
              ...
              <div class="col-lg-12 col-12">
                  <h6 class="text-center"><b><u>Homeroom Teacher Comments & Recommendations</u></b></h6>
                  <div class="row">
                    ...
                    $subALl=$countSubject->num_rows();
                      if($subALl>0){
                        $output.="<b>1<sup>st</sup> Term Homeroom Teacher's Comment</b><br>";
                        $quartrTotal=$this->db->query("select sum(total) as total from reportcard".$gradesec.$max_year." where stuid='$stuid' and quarter='Term1' and onreportcard='1' and letter='#' ");
                        if($quartrTotal->num_rows()>0){
                          foreach ($quartrTotal->result() as $totalValue) {
                            ...
                            if($printValue1 >0 && $reportCardComments->num_rows()>0){
                              ...
                              $output.='________________________________________________________________ <br>';
                            }
                          }
                        }else{
                          $output.='No comment.';
                    ...
                    $output.="
                      <p>Teacher's Name: <u><b>".$tfName." ".$tmName."</b></u> Sig.________________________ Date:____________</p>
                    </div>";

                    $output.='<div class="col-lg-12 col-12 StudentViewTextInfo">';
                    if($subALl>0){
                        $output.="<b>2<sup>nd</sup> Term Homeroom Teacher's Comment</b><br>";
                        $quartrTotal=$this->db->query("select sum(total) as total from reportcard".$gradesec.$max_year." where stuid='$stuid' and quarter='Term2' and onreportcard='1' and letter='#' ");
                        if($quartrTotal->num_rows()>0){
                          foreach ($quartrTotal->result() as $totalValue) {
                    ...
                    $output.='<div class="col-lg-12 col-12 StudentViewTextInfo">';
                    if($subALl>0){
                        $output.="<b>3<sup>rd</sup> Term Homeroom Teacher's Comment</b><br>";
                        $quartrTotal=$this->db->query("select sum(total) as total from reportcard".$gradesec.$max_year." where stuid='$stuid' and quarter='Term3' and onreportcard='1' and letter='#' ");
                        if($quartrTotal->num_rows()>0){
                          foreach ($quartrTotal->result() as $totalValue) {
                    ...
                    $output.="
                      <p>Teacher's Name: <u><b>".$tfName." ".$tmName."</b></u> Sig.________________________ Date:____________</p>
                      Parent's/Guardian's Name: <u><b>".$mname." " .$lname." </b></u> Sig.__________________ Date:____________
                    </div>";

                    $output.='<div class="col-lg-12 col-12 StudentViewTextInfo">';
                    $output.='<p><b>Promoted To Grade_____________.</b></p>';
                    $output.="<b>Director's Name _________________________________________. Sig.____________ </b><br>";
            ...
            <div class="row" id="ENS">
              <div class="col-lg-6 col-6">
                <div class="row">
                  ...
                  <div class="col-lg-12 col-12">
                    <p class="text-center"><i class="fas fa-phone-square"></i> Kindergarten: +251 118931793<p> 
                    <p class="text-center"><i class="fas fa-phone-square"></i> Elementary: +251 116678910<p> 
                    <p class="text-center"><i class="fas fa-phone-square"></i> Middle School: +251 118593609<p> 
                    <p class="text-center"><i class="fas fa-phone-square"></i> High School: +251 118594130<p> 
                    <p class="text-center"><i class="fas fa-phone-square"></i> General Director: +251 930329537<p> 
                    
                    <p class="text-center"><i class="fas fa-envelope"></i> info@diamondacademyeth.com / '.$email.'</p>
  ...
  function customReportCardDiamond($max_year,$gradesec,$branch,$max_quarter,$id,$includeBackPage){
    $output ='';
    $resultSem1=0;
    $resultSem2=0;
    $queryCHK=$this->db->query("select * from reportcard".$gradesec.$max_year." where grade='$gradesec' and academicyear='$max_year' and rpbranch='$branch' and onreportcard='1' group by subject order by subjorder ASC");
...
function kGreportcardByQuarterDiamond($max_year,$gradesec,$branch,$max_quarter,$startDate1,$endDate1,$includeBackPage){
    $querySubject=$this->db->query("select us.grade from users as us where  us.gradesec='$gradesec' and us.academicyear='$max_year' and us.branch='$branch' group by grade ");
    $output='';
    $rowCHK=$querySubject->row();
    $rowGrade=$rowCHK->grade;
    if($rowGrade == 'Kg' || $rowGrade == 'PreKG'|| $rowGrade == 'Prep'){
    /*if($rowGrade == 'KG1' || $rowGrade == 'KG2'||$rowGrade == 'KG3'){*/
      $output ='';
      $queryStudent=$this->db->query(" Select fname,mname,lname,id,grade,gender,age,city,kebele, profile, section,gradesec,username from users where gradesec='$gradesec' and usertype='Student' and branch='$branch' and isapproved='1' and status='Active' and academicyear='$max_year' order by fname,mname,lname ASC ");
      $stuAll=$queryStudent->num_rows();
      ...
      if($queryStudent->num_rows()>0){
        ...
        foreach ($queryStudent->result() as $fetchStudent)
        {
          ...
          $output.= '<div style="width:100%;height:auto;page-break-inside:avoid;display: block; ">
          <div class="row" id="ENS">
          <div class="col-lg-6 col-md-6 StudentViewTextInfo">';
          $output.='<div class="table-responsive">
          <table width="100%" id="ENS" class="tabler table-borderedr table-md" cellspacing="9" cellpadding="9">';
          $output.='<tr><th colspan="5" class="text-center">
          <h6 id="ENScool"><B id="ENS">'.$school_name.' '.$termName.' Student Report Card For '.$gYearName.' G.C ('.$max_year.' E.C) </B></h6>
          </th></tr>';
          $quarterArrayQ=array('Term1','Term2','Term3');
          $output.="<div class='row'><div class='col-lg-6'><b id='ENS'>Grade: ".$fetchStudent->grade."</b></div>";
          $output.="<div class='col-lg-6'><b id='ENS'>Section: ".$fetchStudent->section."</b></div></div>";
          $queryCategory=$this->db->query("select * from bscategory where academicyear='$max_year' and bcgrade='$grade' and bcsubjectrow='1' group by bscategory order by bcorder ASC");
          $queryEvaKey=$this->db->query("select * from bstype where academicyear='$max_year' and btgrade='$grade' ");
          if($queryCategory->num_rows()>0){
            $output.= '<div class="table-responsive">
            <table width="100%" id="ENS" class="tabler table-borderedr table-md" cellspacing="5" cellpadding="5">';
            ...
            foreach ($queryEvaKey->result() as $qvalue) 
            {
              $output .='<th class="text-center">'.$qvalue->bstype.'</th>';
            }
            foreach ($queryCategory->result() as $bscatvalue) {
              $bscategory=$bscatvalue->bscategory;
              ...
              foreach ($query_basicskill->result() as $bsvalue) {
                ...
                {
                  if($query_bsvalue->num_rows()>0) {
                    $rowArray=$query_bsvalue->row_array();
                    $valueArray=$rowArray['value'];
                      if($qvalue->bstype==$valueArray){
          ...
          $output.='<div class="row">';
          $output.='<div class="col-lg-12 col-12">Listed under each skill your child’s progress is evaluated as:</div>';
          foreach ($queryEvaKey->result() as $qvalue) 
            {
              $output.='<div class="col-md-6 col-12">';
              $output.='<p>'.$qvalue->bstype.'=> '.$qvalue->bsdesc.'</p>';
              $output.='</div>';
            }
          $output.='</div>';
          ...
          $output.= '<div class="col-lg-6 col-md-6 StudentViewTextInfo">';
          $output.="<div class='col-lg-12'><b id='ENS'>Student's Name: ".$fetchStudent->fname." ".$fetchStudent->mname." ".$fetchStudent->lname."</b></div>";
          $queryCategory=$this->db->query("select * from bscategory where academicyear='$max_year' and bcgrade='$grade' and bcsubjectrow='0' group by bscategory order by bcorder ASC");
          if($queryCategory->num_rows()>0){
            $output.= '<div class="table-responsive">
            <table width="100%" id="ENS" class="tabler table-borderedr table-md" cellspacing="5" cellpadding="5">';
            $dateYear=date('Y');
            $output .='<tr><th colspan="5" class="text-center"><h6 id="ENScool"><B id="ENS">'.$school_name.' '.$termName.' Student Report Card For '.$gYearName.' G.C ('.$max_year.' E.C) </B></h6></th></tr>';
            $output .='<tr><th>Evaluation Area</th>';
            $queryEvaKey=$this->db->query("select * from bstype where academicyear='$max_year' and btgrade='$grade' ");
            foreach ($queryEvaKey->result() as $qvalue) 
            {
                ...
                <div class="col-lg-6 col-6">
                  <div class="row">
                    <div class="col-lg-12 col-12">';
                      ...
                      $output.='<p><input type="radio" name="neededInthis" > More effort is needed in __________________________________________________. </p>';
                      $output.='<p><input type="radio" name="neededInthis" > Excellent!! Keep up the good work. </p>';
                      $output.='<p><input type="radio" name="neededInthis" > Give serious consideration at home for poor behavior. </p>';
                      $queryHoomRoom=$this->db->query("select u.fname,u.mname from users as u cross join hoomroomplacement as hm where hm.roomgrade='$gradesec' and hm.academicyear='$max_year' and hm.branch='$branch' and u.branch='$branch' and u.username=hm.teacher and u.status='Active' and u.isapproved='1' ");
                      if($queryHoomRoom->num_rows()>0){
                        $rowHommeRoom=$queryHoomRoom->row_array();
                        $tfName=$rowHommeRoom['fname'];
                      ...
                      $output.="<p>Teacher's Name: <u><b>".$tfName." ".$tmName."</b></u> Sig.____________ Date:______</p>
                      <p>Assistance Principal’s Name: <u><b>".$tfName." ".$tmName."</b></u> Sig.____________ Date:______</p>
                      <p>Director’s Name: <u><b>".$tfName." ".$tmName."</b></u> Sig.____________ Date:______</p>
                      <p>Parent’s Name: <u><b>".$mname." ".$lname."</b></u> Sig.____________ Date:______</p>";
                      $output.='<p class="text-center"> If you have any question, please call:<br> 
                      Kindergarten: 011 893 17 93</br>
                      General Director: +251 930 32 95 37</p>
                    </div>
                  </div>
                </div>
                <div class="col-lg-6 col-6">';
                  $output.='<p class="text-center"><img class="text-center" src="'.base_url().'/logo/'.$logo.'" style="height:150px;width:150px;border:1px solid #fff; -webkit-border-radius:10px; -moz-border-radius:10px; -ms-border-radius:10px;border-radius:10px" ></p>
                  <h3>'.$termName.' Report Card for the Academic Year  <b>'.$max_year.' E.C ('.$gYearName.' G.C)</b></h3>
                </div>
              </div>
            </div>';
          }
