...
class Documents extends CI_Controller {
  ...
  function fetchThisTaskName(){
    ...
    if($this->input->post('taskName')){
      ...
      echo $this->main_model->fetchThisTaskName($task_name,$max_year,$user);
    }
  }
  function submitTask(){
    $query = $this->db->query("select max(year_name) as year from academicyear");
    ...
    $max_year=$row->year;
    $user=$this->session->userdata('username');
    if($this->input->post('taskText')){
      $taskText=trim($this->input->post('taskText'));
      $TaskName=$this->input->post('TaskName');
      $data=array(
        'taskName'=>$TaskName,
        'staff_name'=>$user,
        'task_response'=>$taskText,
