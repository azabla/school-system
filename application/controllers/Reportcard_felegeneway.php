    ...
    function Fetch_studentreportcard(){
        ...
        if($this->input->post('gradesec')){
            ...
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1' ){
                ...
                if($query){
                    $data=$this->Reportcard_model->reportcardFelegenewy($reportaca,$gradesec,$branch,$rpQuarter,$includeBackPage);
                    echo json_encode($data);
                }
            }else{
                $query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$mybranch,$rpQuarter);
                if($query){
                    $data=$this->Reportcard_model->reportcardFelegenewy($reportaca,$gradesec,$mybranch,$rpQuarter,$includeBackPage);
                    echo json_encode($data);
                }
            }
        }
    ...
    function fetchThisStudentReportcard(){
        ...
        if($this->input->post('id')){
            ...
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
                $query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$branch,$rpQuarter);
                if($query){
                    $data=$this->Reportcard_model->customReportCardFelegeneway($reportaca,$gradesec,$branch,$rpQuarter,$id,$includeBackPage);
                    echo json_encode($data);
                }
            }else{
                $query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$mybranch,$rpQuarter);
                if($query){
                    $data=$this->Reportcard_model->customReportCardFelegeneway($reportaca,$gradesec,$mybranch,$rpQuarter,$id,$includeBackPage);
                    echo json_encode($data);
                }
            }
        }
