   ...
   function Fetch_quarter_roster(){
      ...
      if($this->input->post('gradesecQuarter')){
         ...
         if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
            echo $this->Reportcard_model->quarterosterFairway($reportaca,$gradesec,$branch,$roQuarter); 
         }else{
            echo $this->Reportcard_model->quarterosterFairway($reportaca,$gradesec,$mybranch,$roQuarter); 
         }
      }
   }
      ...
      if($this->input->post('gradesec')){
         ...
         if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
            $query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$branch,$rpQuarter);
               if($query){
                  echo $this->Reportcard_model->rosterFairwaySemester($reportaca,$gradesec,$branch,$page);
               }
         }else{
            $query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$mybranch,$rpQuarter);
            if($query){
               echo $this->Reportcard_model->rosterFairwaySemester($reportaca,$gradesec,$mybranch,$page); 
            }
         }
      }
   }
