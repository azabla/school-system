...
class Roster extends CI_Controller {
   ...
   function Fetch_quarter_roster(){
      ...
      if($this->input->post('gradesecQuarter')){
         ...
         if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
            ...
            if($query){
                echo $this->Reportcard_model->quarterosterMountolive($reportaca,$gradesec,$branch,$roQuarter); 
            }
         }else{
            $query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$mybranch,$roQuarter);
            if($query){
                echo $this->Reportcard_model->quarterosterMountolive($reportaca,$gradesec,$mybranch,$roQuarter); 
            }
         }
      }
   }
   function Fetchroster(){
      ...
      if($this->input->post('gradesec')){
         ...
         $queryQuarter=$this->db->query("select max(term) as quarter from quarter as q cross join users as us where Academic_year='$max_year' and q.termgrade=us.grade and us.gradesec='$gradesec' and us.academicyear='$max_year' group by us.grade,q.termgroup ");
         ...
         if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
            $query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$branch,$rpQuarter);
               if($query){
                  echo $this->Reportcard_model->rosterMountolive($reportaca,$gradesec,$branch,$page);
               }
         }else{
            $query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$mybranch,$rpQuarter);
            if($query){
               echo $this->Reportcard_model->rosterMountolive($reportaca,$gradesec,$mybranch,$page); 
            }
         }
      }
   }
