   ...
   function Fetch_quarter_roster(){
      ...
      if($this->input->post('gradesecQuarter')){
         ...
         if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
            echo $this->Reportcard_model->quarterosterGreen($reportaca,$gradesec,$branch,$roQuarter); 
         }else{
            echo $this->Reportcard_model->quarterosterGreen($reportaca,$gradesec,$mybranch,$roQuarter); 
         }
      }
   }
            ...
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               $query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$branch,$rpQuarter);
                  if($query){
                     if($termgroups=='Quarter'){
                        ...
                        echo $this->Reportcard_model->rosterGreen($reportaca,$gradesec,$branch,$page);
                     }
                  }
            }else{
               ...
               if($query){
                  if($termgroups=='Quarter'){
                     ...
                     echo $this->Reportcard_model->rosterGreen($reportaca,$gradesec,$mybranch,$page); 
                  }
               }
            }
         }else{
