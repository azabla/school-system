...
class Ngmarkresult extends CI_Controller {
  ...
  function filterGradesecToNGMarkShow(){
    $user=$this->session->userdata('username');
    ...
    if($this->input->post('branchit')){
      ...
      echo $this->main_model->filterGradesecToNGMarkShow($branch,$max_year); 
    }
  }
  function filterSubjectToNGMarkShow(){
    ...
    if($this->input->post('grade2analysis')){
      ...
      if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
          echo $this->main_model->filterSubjectToNGMarkShow($branch,$gradeGsanalysis,$max_year); 
      }else{
  ...
  function fetchNullMark(){
    ...
    if($this->input->post('gs_gradesec')){
      ...
      if(trim($_SESSION['usertype'])===trim('superAdmin') || $accessbranch === '1'){
        echo $this->main_model->outof_error($gs_branches,$grade,$subject,$gs_quarter,$max_year); 
      }else{
