...
class Libraryidcard extends CI_Controller {
    public function __construct(){
        ...
        $userLevel = userLevel();
        $usergroupPermission=$this->db->query("SELECT * from usergrouppermission where usergroup='".$_SESSION['usertype']."' and tableName='IDCard' and allowed='StudentLibraryID' order by id ASC ");
	...
	public function index($page='libraryidcard')
	{
    ...
    function Filter_grade_from_branch_Back(){
        ...
        if($this->input->post('branchit')){
            $branch=$this->input->post('branchit');
            echo $this->main_model->Filter_grade_from_branch_Back($branch,$max_year); 
        }
    }
    ...
    function fetchStudentIdcard(){
        ...
        if($this->input->post('servicePlace')){
            ...
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
                echo $this->main_model->fetch_student_idcard_library($reportaca,$checkStudent,$gradesec,$check,$branch,$gyear); 
            }else{
                echo $this->main_model->fetch_student_idcard_library($reportaca,$checkStudent,$gradesec,$check,$mybranch,$gyear); 
            } 
        }
    }
    function fetchStudentIdcardWithoutPlace(){
        ...
        if($this->input->post('studentList')){
            ...
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
                echo $this->main_model->fetchStudentIdCard_Library($reportaca,$checkStudent,$gradesec,$branch,$gyear);
            }else{
                echo $this->main_model->fetchStudentIdCard_Library($reportaca,$checkStudent,$gradesec,$mybranch,$gyear);
            }
        }
    }
    ...
    function fetchCustomIDCard(){
        ...
        if($this->input->post('stuIdArray')){
            ...
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
                echo $this->main_model->fetchStudentCustomIdCard_Library($max_year,$checkStudent,$gyear);
            }else{
                echo $this->main_model->fetchStudentCustomIdCard_Library($max_year,$checkStudent,$gyear);
            }
        } 
    ...
    public function fetchBackIdCard(){
        ...
        if($this->input->post('pageNumber')){
            ...
            echo $this->main_model->fetchBackIdCard($pageNumber,$max_year,$gyear,$branchIDBack,$gradesecIDBack);
        }
    } 
}