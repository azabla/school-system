...
class Exportbsformate extends CI_Controller {
	...
	public function index($page='exportbsformate')
	{
    ...
    function export(){
        ...
        if(isset($_POST['exportbs'])){
            ...
            $branch1=$this->input->post('branchbs');
            $filename =$gradesec.'.csv';
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
                $listInfo = $this->main_model->export_student_bs_formate($gradesec,$max_year,$branch1);
                $evnameinfo = $this->main_model->export_this_grade_bsname($gradesec,$max_year,$branch1);
            }else{
           ...
           /* $objPHPExcel->getActiveSheet()->SetCellValue('E1','Conduct'); */
            ...
            foreach ($listInfo as $list) {
                ...
                $objPHPExcel->getActiveSheet()->SetCellValue('E' . $rowCount, $branch1);
                $objPHPExcel->getActiveSheet()->SetCellValue('F' . $rowCount, $max_quarter);
                $rowCount++;
            }
