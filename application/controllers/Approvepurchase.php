...
class Approvepurchase extends CI_Controller {
	...
	public function index($page='request-approve')
	{
    ...
    function fetch_myrequested_purchase_toapprove(){
        $user=$this->session->userdata('username');
        echo $this->main_model->fetch_myrequested_purchase_toapprove($user);
    }
    function view_this_purchase_request(){
        if($this->input->post('requestid')){
            $requestid=$this->input->post('requestid',TRUE);
            $requestid = xss_clean($requestid);
            echo $this->main_model->fetch_requested_purchase_list($requestid);
        }
    }
    function approve_purchase_request_item(){
        $user=$this->session->userdata('username');
        if($this->input->post('requestid')){
            $requestid=$this->input->post('requestid',TRUE);
            $requestid = xss_clean($requestid);
            ...
            $query=$this->db->update('stock_purchase_request');
        }
    }
    function fetch_purchase_report(){
        $postData = $this->input->post();
        $data = $this->main_model->fetch_purchase_report($postData);
        echo json_encode($data);
    }
    function view_this_approved_purchase_request(){
        if($this->input->post('requestid')){
            $requestid=$this->input->post('requestid',TRUE);
            $requestid = xss_clean($requestid);
            echo $this->main_model->viewdetail_this_approved_purchase_request($requestid);
        }
    }
}
