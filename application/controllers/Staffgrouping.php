...
class Staffgrouping extends CI_Controller {
	...
	public function index($page='staffgrouping')
	{
    ...
    function post_placement(){
        ...
        if(isset($_POST['departmentHead'])){
            ...
            foreach ($staff_list as $staff_lists) {
                $query=$this->main_model->add_staff_grouping($departmentHead,$staff_head,$staff_lists,$max_year);
                if($query){
                    $data[]=array(
                        'staff_head'=>$staff_head,
                        'academicyear'=>$max_year,
                        'staff_list'=>$staff_lists,
                        'department_name'=>$departmentHead,
                        'createdby'=>$user,
                        'date_created'=>date('M-d-Y')
                    );
                }
            }
            $query=$this->db->insert_batch('staff_group',$data);
            if($query){
                echo '1';
            }else{
                echo '0';
