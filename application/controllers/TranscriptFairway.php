...
class Transcript extends CI_Controller {
   ...
   function fetchThisStudentTranscript(){
      ...
      if($this->input->post('id')){
         ...
         if($includeLetterTranscript==1){
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->letterCustomTranscriptFairway($academicyear,$gradesec,$branch,$username,$noGrade); 
            }else{
               echo $this->Reportcard_model->letterCustomTranscriptFairway($academicyear,$gradesec,$mybranch,$username,$noGrade);
            }
         }else{
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->transcriptCustomFairway($academicyear,$gradesec,$branch,$username,$noGrade); 
            }else{
               echo $this->Reportcard_model->transcriptCustomFairway($academicyear,$gradesec,$mybranch,$username,$noGrade);
            }
         }
      }
   }
   function Fetchtranscript(){
      ...
      if($this->input->post('gradesec')){
         ...
         if($includeLetterTranscript==1){
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->letterTranscriptFairway($academicyear,$gradesec,$branch,$noGrade); 
            }else{
               echo $this->Reportcard_model->letterTranscriptFairway($academicyear,$gradesec,$mybranch,$noGrade);
            }
         }else{
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->transcriptFairway($academicyear,$gradesec,$branch,$noGrade); 
            }else{
               echo $this->Reportcard_model->transcriptFairway($academicyear,$gradesec,$mybranch,$noGrade);
            }
         }
      }
   }
