...
class Mystaffsreport extends CI_Controller {
    public function __construct(){
        ...
        if($this->session->userdata('username') == '' || $userpStaffDe->num_rows()<1 || $userLevel!='2'){
	...
	public function index($page='mystaffsreport')
	{
    ...
    function fetch_my_userstasks_report(){
        ...
        if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
            $this->db->where('report_to',$userType);
            $this->db->set('seen_status','1');
            $this->db->update('stafftasks_report');
            echo $this->main_model->fetch_my_userstasks_report($max_year,$userType,$user);
        }else{
            echo $this->main_model->fetch_mybranch_userstasks_report($max_year,$userType,$user,$mybranch);
        }
    }
}