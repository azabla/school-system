...
class Roster extends CI_Controller {
   ...
   function Fetch_quarter_roster(){
      ...
      if($this->input->post('gradesecQuarter')){
         ...
         if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
            echo $this->Reportcard_model->quarterosterCrosspoint($reportaca,$gradesec,$branch,$roQuarter); 
         }else{
   ...
   function Fetchroster(){
      ...
      if($this->input->post('gradesec')){
         ...
         $queryQuarter=$this->db->query("select max(term) as quarter,q.termgroup from quarter as q cross join users as us where Academic_year='$max_year' and q.termgrade=us.grade and us.gradesec='$gradesec' and us.academicyear='$max_year' group by us.grade,q.termgroup ");
         if($queryQuarter->num_rows()>0){
            ...
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               $query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$branch,$rpQuarter);
                  if($query){
                     if($termgroups=='Quarter'){
                        echo $this->Reportcard_model->KgrosterCrosspoint($reportaca,$gradesec,$branch,$page);
                     }else{
                        echo $this->Reportcard_model->rosterCrosspoint($reportaca,$gradesec,$branch,$page);
                     }
                  }
            }else{
               ...
               if($query){
                  if($termgroups=='Quarter'){
                     echo $this->Reportcard_model->KgrosterCrosspoint($reportaca,$gradesec,$mybranch,$page); 
                  }else{
