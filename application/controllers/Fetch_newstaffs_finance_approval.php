...
class Fetch_newstaffs_finance_approval extends CI_Controller 
{
	...
	public function index()
	{
        ...
        if($usertype==trim('superAdmin') ){
            if(isset($_POST['view'])){
                echo $this->main_model->fetch_new_staffs_finance_approval($max_year);
            }else{
                redirect('home/');
            }
        }else{
            if(isset($_POST['view'])){
                echo $this->main_model->fetch_new_staffs_finance_approval_admin($max_year,$branch);
                redirect('home/');
            }
        }
	}    
