...
class Transcript extends CI_Controller {
   ...
   function fetchThisStudentTranscript(){
      ...
      if($this->input->post('id')){
         ...
         if($includeLetterTranscript==1){
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->letterCustomTranscriptDiamond($academicyear,$gradesec,$branch,$username,$noGrade); 
            }else{
               echo $this->Reportcard_model->letterCustomTranscriptDiamond($academicyear,$gradesec,$mybranch,$username,$noGrade);
            }
         }else{
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->transcriptCustomDiamond($academicyear,$gradesec,$branch,$username,$noGrade); 
            }else{
               echo $this->Reportcard_model->transcriptCustomDiamond($academicyear,$gradesec,$mybranch,$username,$noGrade);
            }
         }
      }
   }
   function Fetchtranscript(){
      ...
      if($this->input->post('gradesec')){
         ...
         if($includeLetterTranscript==1){
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->letterTranscriptDiamond($academicyear,$gradesec,$branch,$noGrade); 
            }else{
               echo $this->Reportcard_model->letterTranscriptDiamond($academicyear,$gradesec,$mybranch,$noGrade);
            }
         }else{
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->transcriptDiamond($academicyear,$gradesec,$branch,$noGrade); 
            }else{
               echo $this->Reportcard_model->transcriptDiamond($academicyear,$gradesec,$mybranch,$noGrade);
            }
         }
      }
   }
