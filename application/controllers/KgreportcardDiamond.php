        ...
        if($this->input->post('gradesec')){
            ...
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               /*$query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$branch,$rpQuarter);
                if($query){*/
                    $data=$this->Reportcard_model->kGreportcardByQuarterDiamond($reportaca,$gradesec,$branch,$rpQuarter,$startDate1,$endDate1,$includeBackPage);
                    echo json_encode($data);
                /*}*/
            }else{
               /* $query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$mybranch,$rpQuarter);
                if($query){*/
                    $data=$this->Reportcard_model->kGreportcardByQuarterDiamond($reportaca,$gradesec,$mybranch,$rpQuarter,$startDate1,$endDate1,$includeBackPage);
                    echo json_encode($data);
                /*}*/
            }
        }
