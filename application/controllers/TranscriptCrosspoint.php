...
class Transcript extends CI_Controller {
   ...
   function fetchThisStudentTranscript(){
      ...
      if($this->input->post('id')){
         ...
         if($includeLetterTranscript==1){
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->letterCustomTranscriptCrosspoint($academicyear,$gradesec,$branch,$username,$noGrade); 
            }else{
               echo $this->Reportcard_model->letterCustomTranscriptCrosspoint($academicyear,$gradesec,$mybranch,$username,$noGrade);
            }
         }else{
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->transcriptCustomCrosspoint($academicyear,$gradesec,$branch,$username,$noGrade); 
            }else{
               echo $this->Reportcard_model->transcriptCustomCrosspoint($academicyear,$gradesec,$mybranch,$username,$noGrade);
            }
         }
      }
   }
   function Fetchtranscript(){
      ...
      if($this->input->post('gradesec')){
         ...
         if($includeLetterTranscript==1){
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->letterTranscriptCrosspoint($academicyear,$gradesec,$branch,$noGrade); 
            }else{
               echo $this->Reportcard_model->letterTranscriptCrosspoint($academicyear,$gradesec,$mybranch,$noGrade);
            }
         }else{
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->transcriptCrosspoint($academicyear,$gradesec,$branch,$noGrade); 
            }else{
               echo $this->Reportcard_model->transcriptCrosspoint($academicyear,$gradesec,$mybranch,$noGrade);
            }
         }
      }
   }
