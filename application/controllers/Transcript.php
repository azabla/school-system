...
class Transcript extends CI_Controller {
   ...
   function updateStudentLeavingStatus(){
      ...
      if($this->input->post('reasonIssue')){
         ...
         if($reasonIssue == 'backToDefaultReason'){
            echo $this->Reportcard_model->backToDefaultReason($reasonIssue,$stuid,$academicyear,$user);
         }else{
            echo $this->Reportcard_model->updateStudentLeavingStatus($reasonIssue,$stuid,$academicyear,$user);
         }  
      }
   }
   ...
   function fetchThisStudentTranscript(){
      ...
      if($this->input->post('id')){
         ...
         $queryStudent=$this->db->query("select fname,mname,lname,id,grade, section, gradesec, username, branch from users where username='$username' and academicyear='$academicyear' ");
         $rowStudent=$queryStudent->row();
         $gradesec=$rowStudent->gradesec;
         $branch=$rowStudent->branch;
         if($includeLetterTranscript==1){
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->letterCustomTranscriptNewayChallenge($academicyear,$gradesec,$branch,$username,$noGrade); 
            }else{
               echo $this->Reportcard_model->letterCustomTranscriptNewayChallenge($academicyear,$gradesec,$mybranch,$username,$noGrade);
            }
         }else{
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->transcriptCustomNewayChallenge($academicyear,$gradesec,$branch,$username,$noGrade); 
            }else{
               echo $this->Reportcard_model->transcriptCustomNewayChallenge($academicyear,$gradesec,$mybranch,$username,$noGrade);
            }
         }
      }
   }
   function Fetchtranscript(){
      ...
      if($this->input->post('gradesec')){
         ...
         if($includeLetterTranscript==1){
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->letterTranscriptNewayChallenge($academicyear,$gradesec,$branch,$noGrade); 
            }else{
               echo $this->Reportcard_model->letterTranscriptNewayChallenge($academicyear,$gradesec,$mybranch,$noGrade);
            }
         }else{
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->transcriptNewayChallenge($academicyear,$gradesec,$branch,$noGrade); 
            }else{
               echo $this->Reportcard_model->transcriptNewayChallenge($academicyear,$gradesec,$mybranch,$noGrade);
            }
         }
      }
   }
