    ...
    function Fetch_studentreportcard(){
        ...
        if($this->input->post('gradesec')){
            ...
            if($_SESSION['usertype']===trim('superAdmin')){
                $query=$this->Reportcard_sinziro->update_reportcardResult($reportaca,$gradesec,$branch,$rpQuarter);
                if($query){
                    $data=$this->Reportcard_sinziro->report_card($reportaca,$gradesec,$branch,$rpQuarter);
                    echo json_encode($data);
                }
            }else{
                $query=$this->Reportcard_sinziro->update_reportcardResult($reportaca,$gradesec,$mybranch,$rpQuarter);
                if($query){
                    $data=$this->Reportcard_sinziro->report_card($reportaca,$gradesec,$mybranch,$rpQuarter);
                    echo json_encode($data);
                }
            }
        }
