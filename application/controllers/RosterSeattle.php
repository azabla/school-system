      ...
      if($this->input->post('gradesec')){
         ...
         if($queryQuarter->num_rows()>0){
            ...
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               ...
               if($query){
                  if($termgroups=='Quarter'){
                     if($includeLetterTranscript=="Letter"){
                        echo $this->Reportcard_model->KgrosterSeattleLetter($reportaca,$gradesec,$branch,$page);
                     }else if($includeLetterTranscript=="Both"){
                        echo $this->Reportcard_model->KgrosterSeattle($reportaca,$gradesec,$branch,$page);
                     }else{
                        echo $this->Reportcard_model->KgrosterSeattleNumber($reportaca,$gradesec,$branch,$page);
                     }
                  }else{
                     if($includeLetterTranscript=="Letter"){
                        echo $this->Reportcard_model->rosterSeattleLetter($reportaca,$gradesec,$branch,$page);
                     }else if($includeLetterTranscript=="Both"){
                        echo $this->Reportcard_model->rosterSeattle($reportaca,$gradesec,$branch,$page);
                     }else{
                        echo $this->Reportcard_model->rosterSeattleNumber($reportaca,$gradesec,$branch,$page);
                     }
                  }
               }
            }else{
               ...
               if($query){
                  if($termgroups=='Quarter'){
                     if($includeLetterTranscript=="Letter"){
                        echo $this->Reportcard_model->KgrosterSeattleLetter($reportaca,$gradesec,$mybranch,$page);
                     }else if($includeLetterTranscript=="Both"){
                        echo $this->Reportcard_model->KgrosterSeattle($reportaca,$gradesec,$mybranch,$page);
                     }else{
                        echo $this->Reportcard_model->KgrosterSeattleNumber($reportaca,$gradesec,$mybranch,$page);
                     }
                  }else{
                     if($includeLetterTranscript=="Letter"){
                        echo $this->Reportcard_model->rosterSeattleLetter($reportaca,$gradesec,$mybranch,$page);
                     }else if($includeLetterTranscript=="Both"){
                        echo $this->Reportcard_model->rosterSeattle($reportaca,$gradesec,$mybranch,$page);
                     }else{
                        echo $this->Reportcard_model->rosterSeattleNumber($reportaca,$gradesec,$mybranch,$page);
                     }
                  }
                  /*if($termgroups=='Quarter'){
                     echo $this->Reportcard_model->KgrosterSeattle($reportaca,$gradesec,$mybranch,$page); 
