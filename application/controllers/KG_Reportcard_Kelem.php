        ...
        if($this->input->post('gradesec')){
            ...
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               /*$query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$branch,$rpQuarter);
                if($query){*/
                    $data=$this->Reportcard_model->kGreportcardByQuarterKelem($reportaca,$gradesec,$branch,$rpQuarter,$startDate1,$endDate1,$includeBackPage);
                    echo json_encode($data);
                /*}*/
            }else{
               /* $query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$mybranch,$rpQuarter);
                if($query){*/
                    $data=$this->Reportcard_model->kGreportcardByQuarterKelem($reportaca,$gradesec,$mybranch,$rpQuarter,$startDate1,$endDate1,$includeBackPage);
                    echo json_encode($data);
                /*}*/
            }
        }
    ...
    /*function fetchstudentreportcard(){
        ...
        if($this->input->post('gradesec')){
            ...
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               $query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$branch,$rpQuarter);
                if($query){
                    $data=$this->Reportcard_model->kGreportcardByQuarterDiamond($reportaca,$gradesec,$branch,$rpQuarter,$startDate1,$endDate1);
                    echo json_encode($data);
                }
            }else{
                $query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$mybranch,$rpQuarter);
                if($query){
                    $data=$this->Reportcard_model->kGreportcardByQuarterDiamond($reportaca,$gradesec,$mybranch,$rpQuarter,$startDate1,$endDate1);
                    echo json_encode($data);
                }
            }
        }
    }*/
    /*function fetchKgStudentAnnualReportCard(){
        $user=$this->session->userdata('username');
        $query = $this->db->query("select max(year_name) as year from academicyear");
        $row = $query->row();
        $max_year=$row->year;
        ...
        if($this->input->post('gradesec')){
            ...
            $reportaca=$this->input->post('reportaca');
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
                $query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$branch,$max_quarter);
                if($query){
                    $data=$this->Reportcard_model->Kgannualreportcard($reportaca,$gradesec,$branch,$max_quarter,$startDate1,$endDate1);
                    echo json_encode($data);
                }
            }else{
                $query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$mybranch,$max_quarter);
                if($query){
                    $data=$this->Reportcard_model->Kgannualreportcard($reportaca,$gradesec,$mybranch,$max_quarter,$startDate1,$endDate1);
                    echo json_encode($data);
                }
            }
        }
    }*/
    /*function fetchKgStudentAnnualReportCard2(){
        $user=$this->session->userdata('username');
        $query = $this->db->query("select max(year_name) as year from academicyear");
        $row = $query->row();
        $max_year=$row->year;
        ...
        if($this->input->post('gradesec')){
            ...
            $branch=$this->input->post('branch');
            $reportaca=$this->input->post('reportaca');
            $queryUser=$this->db->query("select grade from users where academicyear='$max_year' and gradesec='$gradesec' ");
            $uRow=$queryUser->row();
            $uGrade =$uRow->grade;
            ...
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
                $query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$branch,$max_quarter);
                if($query){
                    $data=$this->Reportcard_model->KgannualreportcardFinal($reportaca,$gradesec,$branch,$max_quarter,$startDate1,$endDate1);
                    echo json_encode($data);
                }
            }else{
                $query=$this->Reportcard_model->update_reportcardResult($reportaca,$gradesec,$mybranch,$max_quarter);
                if($query){
                    $data=$this->Reportcard_model->KgannualreportcardFinal($reportaca,$gradesec,$mybranch,$max_quarter,$startDate1,$endDate1);
                    echo json_encode($data);
