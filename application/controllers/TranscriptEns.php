...
class Transcript extends CI_Controller {
   ...
   function fetchThisStudentTranscript(){
      ...
      if($this->input->post('id')){
         ...
         if($includeLetterTranscript==1){
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->letterCustomTranscriptENS($academicyear,$gradesec,$branch,$username,$noGrade); 
            }else{
               echo $this->Reportcard_model->letterCustomTranscriptENS($academicyear,$gradesec,$mybranch,$username,$noGrade);
            }
         }else{
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->transcriptCustomENS($academicyear,$gradesec,$branch,$username,$noGrade); 
            }else{
   ...
   function Fetchtranscript(){
      ...
      if($this->input->post('gradesec')){
         ...
         if($includeLetterTranscript==1){
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->letterTranscriptENS($academicyear,$gradesec,$branch,$noGrade); 
            }else{
               echo $this->Reportcard_model->letterTranscriptENS($academicyear,$gradesec,$mybranch,$noGrade);
            }
         }else{
            if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
               echo $this->Reportcard_model->transcriptENS($academicyear,$gradesec,$branch,$noGrade); 
            }else{
