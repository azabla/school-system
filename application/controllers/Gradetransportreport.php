...
class Gradetransportreport extends CI_Controller {
	...
	public function index($page='gradetransportreport')
    {
    ...
    function filterServicePlace(){
        ...
        if($this->input->post('branchit')){
            ...
            echo $this->main_model->fetch_servicePlace_branchReport($branch,$grands_academicyear); 
        }
    }
    function filterGrade(){
        ...
        if($this->input->post('branchit')){
            $branch=$this->input->post('branchit');
            echo $this->main_model->fetch_thisgrade_from_branchNow($branch,$max_year); 
        }
    }
        ...
        if($this->input->post('summaryGSGrade')){
            ...
            if($includeHeader==='1'){
                if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
                    echo $this->main_model->fetch_thisTransportService($gs_branches,$checkedGrade,$checkedPlace,$grands_academicyear);
                }else{
                    echo $this->main_model->fetch_thisTransportService($branch,$checkedGrade,$checkedPlace,$grands_academicyear);
                }
            }else{
                if($_SESSION['usertype']===trim('superAdmin') || $accessbranch === '1'){
                    echo $this->main_model->fetch_thisTransportService_count($gs_branches,$checkedGrade,$checkedPlace,$grands_academicyear,$countPlace);
                }else{
                    echo $this->main_model->fetch_thisTransportService_count($branch,$checkedGrade,$checkedPlace,$grands_academicyear,$countPlace);
                }
            }
        }
    }
