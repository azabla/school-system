...
class Myannualrequest extends CI_Controller {
  ...
  function submit_request(){
	...
	function save_new_leaving_reason(){
    $user=$this->session->userdata('username');
    $usertype=$this->session->userdata('usertype');
    $query = $this->db->query("select max(year_name) as year from academicyear");
    $row = $query->row();
    $max_year=$row->year;
    if($this->input->post('leavingReasonName')){
      $leavingReasonName=trim($this->input->post('leavingReasonName'));
      $data=array(
        'reason_name'=>$leavingReasonName,
        'createdby'=>$user,
        'date_created'=>date('M-d-Y')
      );
      $query=$this->main_model->register_new_leaving_reason($data,$leavingReasonName);
      if($query){
        echo '1';
      }else{
        echo '0';
    ...
    echo $this->main_model->fetch_leaving_reason();
  }
  function removeBook_this_leaving_reason(){
    $user=$this->session->userdata('username');
    if($this->input->post('userid')){
      $userid=$this->input->post('userid');
      $this->db->where('id',$userid);
      $query=$this->db->delete('staff_leaving_reason');
      if($query){
        echo '1';
      }else{
        echo '0';
